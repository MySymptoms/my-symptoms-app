...
export const DialInput: FC<Props> = ({initialValue, ...props}) => {
  const [initValue] = useState(initialValue);

  return <DialInputInner {...props} initialValue={initValue} />;
};

export const DialInputInner = React.memo(
  ({
    initialValue = 0,
    ...
    const [initialRotationRad] = useState(
      toRadScale((initialValue - start) / (end - start)),
    );
        ...
        <Animated.Code>
          {() => {
            ...
            return block([
              ...
              cond(eq(state, State.BEGAN), set(rotateBegin, angle)),
              cond(
                ...
                block([
                  call(
                    [
                      interpolate(
                        modulo(
                          add(rotateEnd, sub(angle, rotateBegin)),
                          2 * Math.PI,
                        ),
                        {
              ...
              cond(
                eq(state, State.END),
                block([
                  set(rotateEnd, add(rotateEnd, sub(angle, rotateBegin))),
                  call(
                    [
                      interpolate(
                        modulo(
